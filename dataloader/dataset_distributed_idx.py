...
class SemKITTI_pt(data.Dataset):
    def __init__(self, data_path, cfgs, split='train', return_ref=False):
        ...
        # gh
        self.root_sam = data_path.replace('SemanticKitti',
                                          'SemKITTI_processed')  # '/home/poscoict/Desktop/SemKITTI_processed/dataset/sequences'
        ...
        weights = np.zeros((len(semkittiyaml['learning_map_inv']) + 1,),
                           dtype=np.float32)  # 这里需要预测第20类（noise），所以weights数量需要+1
        ...
        for seq in self.seqs:
            ...
            for pcd_name in sorted(path_list):
                ...
                self.sam_imgidx_files.append(
                    os.path.join(self.root_sam, seq, 'img_fea', str(pcd_name[:-4]) + '_idx.pt'))
                self.sam_segidx_files.append(
                    os.path.join(self.root_sam, seq, 'seg_fea', str(pcd_name[:-4]) + '_idx.pt'))
    ...
    def __getitem__(self, index):
        ...
        ### us start
        segidx_ = np.copy(segidx) + 1
        segidx_[segidx_ == 256] = 0
        mask_idx = np.zeros((len(pts),), dtype=int)
        count = 0
        for i in range(len(pts)):
            if mask1[i] == True:
                mask_idx[i] = segidx_[2, count]  # mask 0 -> 2
                count += 1

        pts = np.concatenate((pts, np.expand_dims(mask_idx, axis=1)), axis=1) # mask 0 -> 2
        ### us end

        segfea_ = torch.zeros((len(segfea) + 1, 256))
        segfea_[:-1] = segfea
        segidx[segidx == 255] = len(segfea)  # (3, n pts)
        ...
        data_tuple = (pts[:, :3], pts[:, 3:], self.tokens[index])  # us
        if self.split in ["train", "val"]:
...
class spherical_dataset(data.Dataset):
        ...
        if self.inst_aug:
            ...
            if cfgs['dataset']['inst_aug']['aug_type'] == "contmix":
                self.copy_paste = Cont_Mix_InstAugmentation(dataset_name=cfgs["dataset"]["name"],
                                                            instance_pkl_path=cfgs['dataset']['inst_aug'][
                                                                'inst_pkl_path'],
    ...
    def __getitem__(self, index):
        ...
        feat = np.concatenate((feat, np.expand_dims(mask_idx, axis=1)), axis=1)  # us
        ...
        if self.attn is False:
            ...
            return_dict['segfea'] = fusion_tuple[0].mean(1)
