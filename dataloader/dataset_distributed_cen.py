...
class SemKITTI_pt(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_center = np.zeros((len(segfea) + 1, 3))
        mask_count = np.zeros((len(segfea) + 1,), dtype=int)

        for i in range(1, len(mask_center)):
            id = np.where(mask_idx == i)[0]  # [point indexes with mask i]
            mask_center[i] = np.sum(pts[id, :3], axis=0)
            mask_count[i] = len(id)
        mask_count[mask_count == 0] = 1
        mask_center /= mask_count.reshape(len(mask_count), 1)

        xyz_center = np.zeros((len(pts), 3))
        for i in range(len(pts)):
            xyz_center[i] = mask_center[mask_idx[i]]

        pts = np.concatenate((pts, xyz_center), axis=1)
        ### us end
...
class spherical_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        xyz_center_pol = cart2polar(xyz_center) # us
        ...
        return_fea = np.concatenate((return_fea, xyz_center_pol), axis=1)  # us
